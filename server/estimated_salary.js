...
const fileData = fs.readFileSync(
  path.join(__dirname, "dist", "jobListings.json")
);
...
function getEstimatedSalary() {
  ...
  jobListings.map((job) => {
    ...
    const existingjobSectors = Object.keys(salary_by_job).map((title) =>
      title.toLowerCase()
    );
  ...
  fs.writeFileSync(
    path.join(__dirname, "..", "frontend", "Public", "salaries.json"),
    FinalData
  );
}
